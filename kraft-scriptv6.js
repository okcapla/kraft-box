...
const settings = {
  ...
  function PriceFormula(L, W, H, Q, optLength = 0.0, optWidth = 0.0) {
    ...
    if (pricingModel == "Basic")
      basePrice = (bp / AmountMade * mpm) * dielineMarkup;
  
    if (pricingModel == "InitialCurve") {
      if (InitialFunctionValue < (bp / AmountMade) * mpm)
        basePrice = ((bp / AmountMade) * mpm) * dielineMarkup;
      else
        basePrice = ((InitialFunctionValue + (bp / AmountMade) * mpm) / 2) * dielineMarkup;
    }
  
    if (pricingModel == "GoodCurve") {
      if (InitialFunctionValue * interpolationReductionScale < (bp / AmountMade) * mpm)
        basePrice = ((bp / AmountMade) * mpm) * dielineMarkup;
      else
        basePrice = ((InitialFunctionValue * interpolationReductionScale + (bp / AmountMade) * mpm) / 2) * dielineMarkup;
    }
